...
from src.strategies.manual.tmp_test_ict_multi_setup_v458 import load_nq_data
...
def main() -> None:
    ...
    bt = Backtest(df, ICT_MULTI_SETUP_V458, cash=1_000_000, commission=0.0, exclusive_orders=False, trade_on_close=False)
    stats = bt.run()
    ...
    analysis_meta = meta[(meta.get("setup_type", "").fillna("") != "") | (meta.get("bridge_type", "").fillna("") != "") | (meta.get("setup_tier", "").fillna("") != "")].copy() if {"setup_type","bridge_type","setup_tier"}.issubset(meta.columns) else meta.copy()
    if analysis_meta.empty:
        analysis_meta = meta.copy()
    ...
    ] if c in analysis_meta.columns]
    print(analysis_meta[cols].tail(40).to_string(index=False))

    print("\n5) By setup_type")
    print(summarize_group(analysis_meta, ["setup_type"]).to_string(index=False))

    print("\n6) By bridge_type")
    print(summarize_group(analysis_meta, ["bridge_type"]).to_string(index=False))

    if "setup_type" in meta.columns and "bridge_type" in meta.columns:
        print("\n7) By setup_type + bridge_type")
        print(summarize_group(analysis_meta, ["setup_type", "bridge_type"]).to_string(index=False))

    if "setup_tier" in meta.columns:
        print("\n8) By setup_tier")
        print(summarize_group(analysis_meta, ["setup_tier"]).to_string(index=False))

    print("\n9) Direction split")
    print(summarize_group(analysis_meta, ["side"]).to_string(index=False))

    print("\n10) Entry variant")
    print(summarize_group(analysis_meta, ["entry_variant"]).to_string(index=False))

    if "exit_month_et" in meta.columns:
        monthly = (
            analysis_meta.groupby("exit_month_et", dropna=False)
            .agg(
                trades=("realized_dollars_5_mnq", "size"),
