...
def build_session_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["is_nypm"] = (
        ...
        | (out["et_hour"] == 14)
        | ((out["et_hour"] == 15) & (out["et_minute"] <= 30))
    )
...
class ICT_MULTI_SETUP_V4(Strategy):
    risk_multiple = 2.2
    ...
    def _bear_context_continuation(self, row: pd.Series) -> bool:
        return bool(
            row["is_nyam"]
    ...
    def _bull_execution_ok(self, row: pd.Series) -> tuple[bool, str]:
        if not bool(row["is_nyam_entry_window"]):
            return False, ""
    ...
    def next(self):
        ...
        if self.state.direction == "long":
            ...
            if self.state.context_bar < 0:
                if self.enable_reversal and self._bull_context_reversal(row):
                    self.state.context_bar = i
                    self.state.setup_type = "NYAM_REVERSAL"
                    self.debug_counts["confirm_long_context"] += 1
                    self._sync_debug()
                elif self.enable_continuation and self._bull_context_continuation(row):
                    self.state.context_bar = i
            ...
            if ok:
                ...
                if stop < entry_trigger:
                    ...
                    target = self._target_above(i, entry_trigger, risk)
        ...
        if self.state.direction == "short":
            ...
            if self.state.context_bar < 0:
                if self.enable_reversal and self._bear_context_reversal(row):
                    self.state.context_bar = i
                    self.state.setup_type = "NYAM_REVERSAL"
                    self.debug_counts["confirm_short_context"] += 1
                    self._sync_debug()
                elif self.enable_continuation and self._bear_context_continuation(row):
                    self.state.context_bar = i
                    self.state.setup_type = "NYAM_CONTINUATION"
                    self.debug_counts["confirm_short_context"] += 1
                    self._sync_debug()
            ...
            if ok:
                ...
                if stop > entry_trigger:
                    ...
                    target = self._target_below(i, entry_trigger, risk)
