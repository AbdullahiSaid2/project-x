"""
Corrected top/bottom ticking backtest engine.
...
Install path:
    trading_system/src/strategies/manual/top_bottom_ticking_corrected_backtest_engine.py
...
def _build_guarded_strategy_class(cfg: InstrumentConfig, variant_name: str, base_cls: type, prop_profile_name: str) -> type:
    ...
    class CorrectedStrategy(base_cls):
        ...
        def _record_open_trade_meta_compat(self, entry: float, row: pd.Series):
            """Call the underlying base strategy metadata recorder safely.

            Important: do not call self._record_open_trade_meta_compat() here; that
            would recurse. Call base_cls._record_open_trade_meta directly because
            base_cls is the actual ICT strategy class being wrapped.
            """
            recorder = getattr(base_cls, "_record_open_trade_meta", None)
            if recorder is None:
                return None
            try:
                return recorder(self, entry, row)
            except TypeError as exc:
                msg = str(exc)
                if "positional" in msg or "argument" in msg or "given" in msg:
                    return recorder(self, entry)
                raise
        ...
        def _safe_enter_short(self, row: pd.Series):
            ...
            target3 = entry - (risk * float(self.target3_r))
            ...
            self.be_moved = False
            self._record_open_trade_meta_compat(entry, row)
    ...
    CorrectedStrategy.__name__ = f"{base_cls.__name__}_{cfg.symbol}_{variant_name}_{prop_profile_name}_corrected"
...
def prepare_trade_log(meta: pd.DataFrame, cfg: InstrumentConfig, variant_name: str, prop_profile_name: str) -> pd.DataFrame:
    if meta.empty:
        return meta
...
def run_symbol_variant(cfg: InstrumentConfig, variant_name: str, base_cls: type, prop_profile_name: str):
    ...
    meta = prepare_trade_log(meta, cfg, variant_name, prop_profile_name)
    ...
    if not debug.empty:
        ...
        debug.insert(3, "days_back", cfg.days_back)
        debug.insert(4, "tail_rows", tail_label)
    return stats, meta, debug


def run_all_symbols(symbols: List[str], variants: List[str], prop_profile_name: str, instrument_overrides: Dict[str, InstrumentConfig]):
...
def _suffix_for_args(prop_profile: str, days_back: int, tail_rows: Optional[int]) -> str:
    tail_tag = "notail" if tail_rows is None else f"tail{tail_rows}"
    return f"_{prop_profile}_{days_back}d_{tail_tag}_corrected"
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if not args.filtered_only:
        _, raw_trades, raw_debug = run_all_symbols(symbols, variants, "none", instrument_overrides)
        if not raw_trades.empty:
            outputs = write_report_set(raw_trades, "raw", suffix)
            report_sets.append(("raw", raw_trades, outputs))
            print(f"\nRaw corrected realized PnL: ${raw_trades['realized_pnl_dollars'].sum():.2f}")
        else:
    ...
    if not args.raw_only:
        _, filtered_trades, filtered_debug = run_all_symbols(symbols, variants, args.prop_profile, instrument_overrides)
        if not filtered_trades.empty:
            outputs = write_report_set(filtered_trades, "filtered", suffix)
            report_sets.append(("filtered", filtered_trades, outputs))
            print(f"\nFiltered corrected realized PnL: ${filtered_trades['realized_pnl_dollars'].sum():.2f}")
        else:
    ...
    if len(report_sets) >= 2:
        ...
        for label, trades, _ in report_sets:
            positions = build_position_log(trades)
            portfolio_rows.append(build_portfolio_summary(trades, positions, label))
            audit_rows.append(build_audit_summary(trades, label))
    ...
    print("\nFinished corrected backtest run.")
