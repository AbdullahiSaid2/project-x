...
def load_nq_data() -> pd.DataFrame:
    ...
    # IMPORTANT:
    # V452/V454/V455 expect a UTC-naive DatetimeIndex because parent code does:
    # pd.DatetimeIndex(out.index).tz_localize("UTC")
    #
    # Your parquet is already tz-aware, so we normalize it here to UTC-naive.
...
def main() -> None:
    ...
    print("\n5) Debug counters...")
    for k, v in dict(ICT_MULTI_SETUP_V455.DEBUG_COUNTERS).items():
        print(f"{k}: {v}")
