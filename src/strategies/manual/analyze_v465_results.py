...
from src.strategies.manual.ict_multi_setup_v465 import ICT_MULTI_SETUP_V465
from src.strategies.manual.reporting_v465 import load_strategy_meta
from src.strategies.manual.tmp_test_ict_multi_setup_v465 import load_nq_data


def main() -> None:
    df = load_nq_data()
    print("2) Running backtest...\n")
    bt = Backtest(df, ICT_MULTI_SETUP_V465, cash=1_000_000, commission=0.0, exclusive_orders=False, trade_on_close=False)
    stats = bt.run()
    print("3) Headline stats")
    print(stats)

    meta = load_strategy_meta(ICT_MULTI_SETUP_V465)
    if meta.empty:
