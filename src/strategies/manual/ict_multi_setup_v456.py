...
class ICT_MULTI_SETUP_V456(ICT_MULTI_SETUP_V454):
    name = "ICT_MULTI_SETUP_V456"
    ...
    def init(self) -> None:
        super().init()
        self._v456_news_windows: List[NewsWindow] = self._load_news_windows()
        self._v456_open_trade_session: Dict[int, Any] = {}
        self._v456_last_closed_count: int = 0
        self._v456_daily_realized_pnl = defaultdict(float)
        self._v456_apex_session_realized_pnl = defaultdict(float)
        self._v456_cycle_realized_pnl: float = 0.0
        self._v456_best_day_pnl_in_cycle: float = 0.0
        self._v456_equity_peak_est: float = 50000.0
        self._v456_account_balance_est: float = 50000.0
    ...
    def _in_news_block(self, ts_et: pd.Timestamp) -> bool:
        for event in self._v456_news_windows:
    ...
    def _capture_new_open_trade_context(self, now_et: pd.Timestamp) -> None:
        for trade in self._iter_open_trades():
            key = id(trade)
            if key not in self._v456_open_trade_session:
                self._v456_open_trade_session[key] = self._apex_session_date(now_et)

    def _log_newly_closed_trades_v456(self) -> None:
        closed_trades = list(getattr(self, "closed_trades", []))
        if len(closed_trades) <= self._v456_last_closed_count:
            return
        new_trades = closed_trades[self._v456_last_closed_count:]
        self._v456_last_closed_count = len(closed_trades)
        ...
        for trade in new_trades:
            ...
            if exit_time_et is not None:
                ...
                exit_session = self._apex_session_date(exit_time_et)
                self._v456_daily_realized_pnl[exit_day] += pnl
                self._v456_apex_session_realized_pnl[exit_session] += pnl
                self._v456_cycle_realized_pnl += pnl
                self._v456_account_balance_est = 50000.0 + self._v456_cycle_realized_pnl
                self._v456_equity_peak_est = max(self._v456_equity_peak_est, self._v456_account_balance_est)
                self._v456_best_day_pnl_in_cycle = max(
                    self._v456_best_day_pnl_in_cycle,
                    self._v456_daily_realized_pnl[exit_day],
                    self._v456_apex_session_realized_pnl[exit_session],
                )
            ...
            meta = getattr(self, "open_trade_meta", None) or {}
            planned_entry = meta.get("planned_entry_price", None)
            planned_stop = meta.get("planned_stop_price", None)
            planned_target = meta.get("planned_target_price", None)
            stop_points = meta.get("stop_points", None)
            tp_points = meta.get("tp_points", None)
            planned_rr = meta.get("planned_rr", None)
            ...
            if abs(self._v456_cycle_realized_pnl) > 1e-9:
                consistency_ratio = self._v456_best_day_pnl_in_cycle / abs(self._v456_cycle_realized_pnl)

            row = {
                "side": "LONG" if size >= 0 else "SHORT",
                "setup_type": meta.get("setup_type", self._current_parent_setup_type() or ""),
                ...
                "pnl": pnl,
                "cycle_realized_pnl": self._v456_cycle_realized_pnl,
                "best_day_pnl_in_cycle": self._v456_best_day_pnl_in_cycle,
        ...
        if new_trades and not self.position and hasattr(self, "open_trade_meta"):
    ...
    def next(self) -> None:
        ...
        if self._is_after_hard_flat(now_et):
            ...
            self._log_newly_closed_trades_v456()
            return

        for trade in self._iter_open_trades():
            key = id(trade)
            entry_session = self._v456_open_trade_session.get(key)
