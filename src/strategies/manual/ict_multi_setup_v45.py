...
# ICT_MULTI_SETUP_V45
#
# Pragmatic V45 upgrade from V44.
#
# Main upgrades added:
# 1. Entry refinement:
#    - pullback entries into 30m bridge origin for MSS / CISD / iFVG
#    - 1m refinement required around pullback zone
# 2. Trade management:
#    - partial at configurable R
#    - runner to liquidity
#    - breakeven only after confirmation
# 3. Prop firm mode:
#    - daily loss cap
#    - consecutive loss cap
#    - daily trade cap
#    - payout-defense mode
# 4. Better logging and setup metadata
#
# Notes:
# - This file is intentionally self-contained and designed as a clean
#   forward step from the V44 logic you already validated.
# - It preserves the core V44 stack:
#     4H narrative -> session context -> 30m bridge -> 3m execution -> 1m refine
# - It changes entries from mostly breakout-through-trigger to a more ICT-like
#   pullback-into-POI entry, with confirmation.
# ============================================================
...
class ICT_MULTI_SETUP_V45(Strategy):
    # Core trade profile
    risk_multiple = 2.0
    ...
    def _bull_context_reversal_asia(self, row: pd.Series) -> bool:
        return bool(row["is_asia"] and ((row["swept_prev_day_low"] and row["reclaimed_prev_day_low"]) or (row["swept_prior_us_low"] and row["reclaimed_prior_us_low"])))

    def _bear_context_reversal_asia(self, row: pd.Series) -> bool:
        return bool(row["is_asia"] and ((row["swept_prev_day_high"] and row["rejected_prev_day_high"]) or (row["swept_prior_us_high"] and row["rejected_prior_us_high"])))
    ...
    def _bull_context_continuation_nypm(self, row: pd.Series) -> bool:
        return bool(row["is_nypm"] and row["bull_4h_bias"] and row["above_4h_eq"] and (row["bull_disp_30m"] or row["bull_close_strong_30m"] or row["bull_cisd_30m"]))

    def _bear_context_continuation_nypm(self, row: pd.Series) -> bool:
        return bool(row["is_nypm"] and row["bear_4h_bias"] and row["below_4h_eq"] and (row["bear_disp_30m"] or row["bear_close_strong_30m"] or row["bear_cisd_30m"]))
    ...
    def next(self):
        ...
        if self.pending.direction == "long" and not self.position:
            if self._pending_long_ready(row, high_now, low_now) and self._prop_can_trade():
                ...
                if risk > 0:
                    ...
                    self.buy(size=size, sl=float(self.pending.stop_price), tp=float(self.pending.runner_target))
        ...
        if self.pending.direction == "short" and not self.position:
            if self._pending_short_ready(row, high_now, low_now) and self._prop_can_trade():
                ...
                if risk > 0:
                    self.active_risk = risk
                    size = self._effective_size()
                    self.sell(size=size, sl=float(self.pending.stop_price), tp=float(self.pending.runner_target))
                    self.daily_trade_count += 1
                    self.partial_taken = False
                    self.be_moved = False
                    self.entry_bar_idx = i
        ...
        if self.state.direction == "long":
            ...
            if self.state.bridge_bar < 0:
                ...
                bull_ok = bool(row["bull_cisd_30m"] or row["bull_mss_30m"] or bull_ifvg_ok or row["bull_c2_or_c3"])
        ...
        if self.state.direction == "short":
            ...
            if self.state.bridge_bar < 0:
                ...
                bear_ok = bool(row["bear_cisd_30m"] or row["bear_mss_30m"] or bear_ifvg_ok or row["bear_c2_or_c3"])
