...
def main() -> None:
    ...
    if not meta.empty:
        # Drop duplicate rows that can appear from partial/runner bookkeeping in older builds.
    ...
    output_excel = Path("v458_apex_50k_monthly_payout_export.xlsx")
    output_monthly_csv = Path("v458_apex_50k_monthly_summary.csv")
    output_daily_csv = Path("v458_apex_50k_daily_summary.csv")
    output_trades_csv = Path("v458_apex_50k_trade_log.csv")

    excel_meta = meta.copy()
    for col in excel_meta.columns:
        if str(excel_meta[col].dtype).startswith("datetime64[ns,"):
            excel_meta[col] = excel_meta[col].dt.tz_localize(None)
    ...
    with pd.ExcelWriter(output_excel, engine="openpyxl") as writer:
        ...
        excel_meta.to_excel(writer, index=False, sheet_name="trade_log")
