...
def main() -> None:
    ...
    summarize(meta, ["setup_type"], "realized_dollars_dynamic_contracts", "5) By setup_type (actual in-engine fixed 10 MNQ)")
    summarize(meta, ["bridge_type"], "realized_dollars_dynamic_contracts", "6) By bridge_type (actual in-engine fixed 10 MNQ)")
    summarize(meta, ["setup_type", "bridge_type"], "realized_dollars_dynamic_contracts", "7) By setup_type + bridge_type (actual in-engine fixed 10 MNQ)")
    summarize(meta, ["setup_tier"], "realized_dollars_dynamic_contracts", "8) By setup_tier (actual in-engine fixed 10 MNQ)")
    summarize(meta, ["side"], "realized_dollars_dynamic_contracts", "9) Direction split (actual in-engine fixed 10 MNQ)")
    summarize(meta, ["entry_variant"], "realized_dollars_dynamic_contracts", "10) Entry variant (actual in-engine fixed 10 MNQ)")
    ...
    print("\n11) Monthly realized PnL (actual in-engine fixed 10 MNQ)")
