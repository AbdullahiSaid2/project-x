...
class ICT_MULTI_SETUP_V458(ICT_MULTI_SETUP_V455):
    """
    V457 goals:
    ...
    name = "ICT_MULTI_SETUP_V458"

    APEX_SESSION_START_ET = time(18, 0)
    APEX_SESSION_END_ET = time(16, 59)
    HARD_FLAT_ET = time(16, 55)
    GLOBAL_ENTRY_CUTOFF_ET = time(16, 10)
    NYPM_ENTRY_CUTOFF_ET = time(15, 40)

    NEWS_BLOCK_BEFORE_MIN = 3
    NEWS_BLOCK_AFTER_MIN = 3
    ...
    def init(self) -> None:
        super().init()
        self._v458_news_windows: List[NewsWindow] = self._load_news_windows()
        self._v458_open_trade_session: Dict[int, Any] = {}
        self._v458_open_trade_meta_by_key: Dict[int, Dict[str, Any]] = {}
        self._v458_last_closed_count: int = 0
        self._v458_calendar_daily_realized_pnl = defaultdict(float)
        self._v458_apex_session_realized_pnl = defaultdict(float)
        self._v458_cycle_realized_pnl: float = 0.0
        self._v458_best_day_pnl_in_cycle: float = 0.0
        self._v458_equity_peak_est: float = 50000.0
        self._v458_account_balance_est: float = 50000.0
        self._v458_pending_meta_queue: Deque[Dict[str, Any]] = deque()
        self._v458_last_meta_snapshot: Dict[str, Any] | None = None
    ...
    def _in_news_block(self, ts_et: pd.Timestamp) -> bool:
        for event in self._v458_news_windows:
    ...
    def _record_open_trade_meta(self, entry: float):
        ...
        self._v458_pending_meta_queue.append(snapshot)
        self._v458_last_meta_snapshot = snapshot
    ...
    def _capture_new_open_trade_context(self, now_et: pd.Timestamp) -> None:
        ...
        for trade in open_trades:
            key = id(trade)
            if key in self._v458_open_trade_session:
                continue
            self._v458_open_trade_session[key] = self._apex_session_date(now_et)

            entry_price = self._safe_get_trade_entry_price(trade)
            chosen_meta = None
            if self._v458_pending_meta_queue:
                if entry_price is not None:
                    best_idx = None
                    best_diff = float("inf")
                    for idx, candidate in enumerate(self._v458_pending_meta_queue):
                    ...
                    if best_idx is not None and best_diff <= 8.0:
                        chosen_meta = self._v458_pending_meta_queue[best_idx]
                        del self._v458_pending_meta_queue[best_idx]
                if chosen_meta is None:
                    chosen_meta = self._v458_pending_meta_queue.popleft()
            elif self._v458_last_meta_snapshot is not None:
                chosen_meta = dict(self._v458_last_meta_snapshot)

            self._v458_open_trade_meta_by_key[key] = self._merge_meta(chosen_meta)
    ...
    def _lookup_trade_meta(self, trade: Any, entry_time_et: Optional[pd.Timestamp], entry_price: Optional[float]) -> Dict[str, Any]:
        ...
        for key, meta in list(self._v458_open_trade_meta_by_key.items()):
        ...
        if best_meta is None and self._v458_last_meta_snapshot is not None:
            best_meta = self._v458_last_meta_snapshot
    ...
    def _log_newly_closed_trades_v457(self) -> None:
        ...
        if len(closed_trades) <= self._v458_last_closed_count:
            return
        new_trades = closed_trades[self._v458_last_closed_count:]
        self._v458_last_closed_count = len(closed_trades)
        ...
        for trade in new_trades:
            ...
            if exit_time_et is not None:
                ...
                self._v458_calendar_daily_realized_pnl[calendar_exit_day] += pnl
                self._v458_apex_session_realized_pnl[apex_exit_session] += pnl
                self._v458_cycle_realized_pnl += pnl
                self._v458_account_balance_est = 50000.0 + self._v458_cycle_realized_pnl
                self._v458_equity_peak_est = max(self._v458_equity_peak_est, self._v458_account_balance_est)
                self._v458_best_day_pnl_in_cycle = max(
                    self._v458_best_day_pnl_in_cycle,
                    self._v458_calendar_daily_realized_pnl[calendar_exit_day],
                    self._v458_apex_session_realized_pnl[apex_exit_session],
                )
            ...
            if abs(self._v458_cycle_realized_pnl) > 1e-9:
                consistency_ratio = self._v458_best_day_pnl_in_cycle / abs(self._v458_cycle_realized_pnl)
            ...
            row = {
                ...
                "pnl": pnl,
                "cycle_realized_pnl": self._v458_cycle_realized_pnl,
                "best_day_pnl_in_cycle": self._v458_best_day_pnl_in_cycle,
    ...
    def next(self) -> None:
        ...
        for trade in self._iter_open_trades():
            key = id(trade)
            entry_session = self._v458_open_trade_session.get(key)
