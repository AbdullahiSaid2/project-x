...
class ICT_MULTI_SETUP_V463(ICT_MULTI_SETUP_V460):
    """
    V463
    - Keeps the healthy V460 signal engine intact.
    - Implements a strict 5 MNQ / 10 MNQ regime only.
    - Keeps the validated V460 signal engine and maps only weak/strong setups to size.
    ...
    Replacing that with absolute unit counts choked opportunity flow in the prior V463 build.
    ...
    name = "ICT_MULTI_SETUP_V463"

    # User-requested regime:
    # - 5 MNQ for weaker / standard setups
    # - 10 MNQ only for stronger A-tier setups in more liquid sessions
    ...
    STRONG_BRIDGES = {"IFVG", "C2C3", "MSS", "iFVG"}
    ...
    def _pending_dynamic_contracts(self) -> int:
        ...
        strong_bridge_match = bridge_upper in {b.upper() for b in self.STRONG_BRIDGES}
        is_liquid_session = setup_type in self.STRONG_SETUP_TYPES
        is_a_tier = tier == 'A'
        rr_ok = planned_rr is not None and planned_rr >= self.MIN_FAVORABLE_PLANNED_RR_FOR_SIZEUP
        stop_ok = stop_points is not None and stop_points <= self.MAX_STOP_POINTS_FOR_SIZEUP

        # 10 MNQ only for strong A-tier setups in more liquid sessions.
        if is_a_tier and is_liquid_session and strong_bridge_match and rr_ok and stop_ok:
