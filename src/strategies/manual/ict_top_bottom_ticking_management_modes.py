...
def build_model_frame(df: pd.DataFrame, tick_size: float = DEFAULT_TICK_SIZE) -> pd.DataFrame:
    ...
    # Asia window proxy: 18:00-23:59 ET of the same futures session.
...
class ICT_TOP_BOTTOM_TICKING(Strategy):
    ...
    # Apex-style single-entry, bracketed position, 5 micros, no adds by default.
    #
    # Supported trade-management modes:
    #   partial_contracts   = current/live style: close 2/5 at T1, close 2/3 of remaining at T2, runner to T3.
    #   full_contract_trail = keep full position size, move SL to T1 when T1 is touched, then to T2 when T2 is touched, TP at T3.
    #   no_partial         = no partials/trailing management; full position exits at no_partial_target_r.
    trade_management_mode = "partial_contracts"
    partial_1_fraction = 2 / 5
    partial_2_fraction_of_remaining = 2 / 3
    move_stop_to_be_after_t1 = True

    # Target scheme. These are overridable by the backtest wrapper/CLI.
    target1_r = 1.0
    target2_r = 2.5
    target3_r = 4.0
    no_partial_target_r = 2.0
    ...
    def _record_open_trade_meta(self, entry: float, row: pd.Series | None = None):
        self.open_trade_meta = {
            ...
            "trade_management_mode": str(getattr(self, "trade_management_mode", "partial_contracts")),
            "target1_r": float(getattr(self, "target1_r", np.nan)),
            "target2_r": float(getattr(self, "target2_r", np.nan)),
            "target3_r": float(getattr(self, "target3_r", np.nan)),
            "no_partial_target_r": float(getattr(self, "no_partial_target_r", np.nan)),
        }
    ...
    def _pending_short_ready(self, row: pd.Series, i: int):
        if self.require_internal_sweep_filter and not bool(row.get("internal_sweep_long", False)):
    ...
    def _management_mode(self) -> str:
        mode = str(getattr(self, "trade_management_mode", "partial_contracts")).strip().lower()
        aliases = {
            "partials": "partial_contracts",
            "partial": "partial_contracts",
            "partial_contract": "partial_contracts",
            "full_trail": "full_contract_trail",
            "trail": "full_contract_trail",
            "trail_full": "full_contract_trail",
            "no_partials": "no_partial",
            "none": "no_partial",
            "tp2r": "no_partial",
            "no_partial_2r": "no_partial",
        }
        return aliases.get(mode, mode)

    def _active_target3_r(self) -> float:
        if self._management_mode() == "no_partial":
            return float(getattr(self, "no_partial_target_r", 2.0))
        return float(getattr(self, "target3_r", 4.0))

    def _target_prices_from_meta(self, trade):
        meta = self.open_trade_meta or {}
        entry = float(trade.entry_price)
        risk = float(self.active_risk)
        if self.position.is_long:
            t1 = float(meta.get("planned_target1_price", entry + risk * float(self.target1_r)))
            t2 = float(meta.get("planned_target2_price", entry + risk * float(self.target2_r)))
            t3 = float(meta.get("planned_target3_price", entry + risk * self._active_target3_r()))
        else:
            t1 = float(meta.get("planned_target1_price", entry - risk * float(self.target1_r)))
            t2 = float(meta.get("planned_target2_price", entry - risk * float(self.target2_r)))
            t3 = float(meta.get("planned_target3_price", entry - risk * self._active_target3_r()))
    ...
    def _bar_hit_target1(self, trade) -> bool:
        t1, _, _ = self._target_prices_from_meta(trade)
        high = float(self.data.High[-1])
        low = float(self.data.Low[-1])
        return high >= t1 if self.position.is_long else low <= t1

    def _bar_hit_target2(self, trade) -> bool:
        _, t2, _ = self._target_prices_from_meta(trade)
        high = float(self.data.High[-1])
        low = float(self.data.Low[-1])
        return high >= t2 if self.position.is_long else low <= t2

    def _move_stop_to_price(self, trade, new_stop: float, counter_key: str):
        try:
            if trade.sl is None:
                trade.sl = float(new_stop)
            elif self.position.is_long:
                trade.sl = max(float(trade.sl), float(new_stop))
            else:
                trade.sl = min(float(trade.sl), float(new_stop))
            self.debug_counts[counter_key] = self.debug_counts.get(counter_key, 0) + 1
            self._sync_debug()
        except Exception:
            pass
    ...
    def _enter_short(self, row: pd.Series, i: int):
        ...
        if risk <= 0:
            self._clear_pending()
            return
        # Re-anchor targets to the actual executed entry so TP remains below entry for shorts.
        self.pending.target1 = entry - (risk * self.target1_r)
        self.pending.target2 = entry - (risk * self.target2_r)
        self.pending.target3 = entry - (risk * self._active_target3_r())
        if not (self.pending.target3 < entry < stop):
            self._clear_pending()
            return
        self.active_risk = risk
        self.sell(size=self.fixed_size, sl=stop, tp=float(self.pending.target3))
        self.partial1_taken = False
        self.partial2_taken = False
        self.be_moved = False
    ...
    def _enter_long(self, row: pd.Series, i: int):
        ...
        if risk <= 0:
            self._clear_pending()
            return
        # Re-anchor targets to the actual executed entry so TP remains above entry for longs.
        self.pending.target1 = entry + (risk * self.target1_r)
        self.pending.target2 = entry + (risk * self.target2_r)
        self.pending.target3 = entry + (risk * self._active_target3_r())
        if not (stop < entry < self.pending.target3):
            self._clear_pending()
            return
        self.active_risk = risk
        self.buy(size=self.fixed_size, sl=stop, tp=float(self.pending.target3))
        self.partial1_taken = False
        self.partial2_taken = False
        self.be_moved = False
        self._record_open_trade_meta(entry)
    ...
    def _manage_trade(self):
        ...
        mode = self._management_mode()

        # No-partial mode relies purely on the initial bracket TP/SL.
        # For this mode target3 is redefined to no_partial_target_r during entry.
        if mode == "no_partial":
            return

        # Price-aware target detection: use candle High/Low, not Close.
        # This prevents missed partials/trailing updates when a wick touches T1/T2
        # but the candle closes away from the target.
        hit_t1 = self._bar_hit_target1(trade)
        hit_t2 = self._bar_hit_target2(trade)
        t1, t2, _ = self._target_prices_from_meta(trade)

        if mode == "full_contract_trail":
            # Keep full size. Once T1 is touched, lock stop at T1. Once T2
            # is touched, lock stop at T2. Final TP remains T3.
            if hit_t1 and not self.partial1_taken:
                self._move_stop_to_price(trade, t1, "trail_to_t1")
                self.partial1_taken = True
            if hit_t2 and not self.partial2_taken:
                self._move_stop_to_price(trade, t2, "trail_to_t2")
                self.partial2_taken = True
            return

        # Default/current mode: partial contracts.
        # 5 contracts usually means: 2 off at T1, 2 off at T2, 1 runner to T3.
        if hit_t1 and not self.partial1_taken:
            try:
        ...
        if hit_t2 and self.partial1_taken and not self.partial2_taken:
            try:
