...
def build_session_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Prior US premarket/session highs and lows for NYAM
    ...
    # NYAM levels for NYPM references
    nyam = out[
...
class ICT_MULTI_SETUP_V43(Strategy):
    risk_multiple = 2.0
    ...
    def next(self):
        ...
        if self.state.direction == "long":
            ...
            if self.state.context_bar < 0:
                ...
                if self.enable_asia and self.enable_reversal and self._bull_context_reversal_asia(row):
                    self.state.context_bar = i
                    self.state.setup_type = "ASIA_REVERSAL"
                    context_hit = True
                elif self.enable_asia and self.enable_continuation and self._bull_context_continuation_asia(row):
                    self.state.context_bar = i
                    self.state.setup_type = "ASIA_CONTINUATION"
                    context_hit = True
                elif self.enable_london and self.enable_reversal and self._bull_context_reversal_london(row):
                    self.state.context_bar = i
                    self.state.setup_type = "LONDON_REVERSAL"
                    context_hit = True
                elif self.enable_london and self.enable_continuation and self._bull_context_continuation_london(row):
                    self.state.context_bar = i
                    self.state.setup_type = "LONDON_CONTINUATION"
                    context_hit = True
                elif self.enable_nyam and self.enable_reversal and self._bull_context_reversal_nyam(row):
                    self.state.context_bar = i
        ...
        if self.state.direction == "short":
            ...
            if self.state.context_bar < 0:
                ...
                if self.enable_asia and self.enable_reversal and self._bear_context_reversal_asia(row):
                    self.state.context_bar = i
                    self.state.setup_type = "ASIA_REVERSAL"
                    context_hit = True
                elif self.enable_asia and self.enable_continuation and self._bear_context_continuation_asia(row):
                    self.state.context_bar = i
                    self.state.setup_type = "ASIA_CONTINUATION"
                    context_hit = True
                elif self.enable_london and self.enable_reversal and self._bear_context_reversal_london(row):
                    self.state.context_bar = i
                    self.state.setup_type = "LONDON_REVERSAL"
                    context_hit = True
                elif self.enable_london and self.enable_continuation and self._bear_context_continuation_london(row):
                    self.state.context_bar = i
                    self.state.setup_type = "LONDON_CONTINUATION"
                    context_hit = True
                elif self.enable_nyam and self.enable_reversal and self._bear_context_reversal_nyam(row):
                    self.state.context_bar = i
                    self.state.setup_type = "NYAM_REVERSAL"
                    context_hit = True
