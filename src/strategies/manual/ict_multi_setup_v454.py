...
class ICT_MULTI_SETUP_V454(ICT_MULTI_SETUP_V452):
    """
    V45.4
    ...
    prop_daily_max_trades = 6
    prop_max_consecutive_losses = 4
    ...
    prop_consistency_soft_warning = 0.55
    prop_consistency_hard_block = 0.80
    prop_qualifying_day_target = 150.0
    prop_required_qualifying_days = 5
    prop_soft_lock_after_qualifying = False
    prop_day_a_only_trigger = 500.0
    prop_day_stop_trigger = 1000.0
    prop_red_day_a_only_trigger = -300.0
    ...
    def init(self):
        ...
        self.debug_counts.update({
            ...
            "prop_block_day_target": 0,
            "prop_block_red_day": 0,
        })
        self._sync_debug()
    ...
    def _day_allows_tier(self, tier: str) -> bool:
        if not self.prop_mode:
            return True
        if self.realized_pnl_today <= self.prop_red_day_a_only_trigger and tier != "A":
            self.debug_counts["prop_block_red_day"] += 1
            self._sync_debug()
            return False
        if self.realized_pnl_today >= self.prop_day_a_only_trigger and tier != "A":
            self.debug_counts["prop_block_day_target"] += 1
            self._sync_debug()
            return False
        return True
    ...
    def _arm_long_pullback(self, row: pd.Series, high_now: float, low_now: float, atr3: float, i: int):
        ...
        if not self._day_allows_tier(tier):
            self._clear_state()
            return
