...
from src.strategies.manual.ict_multi_setup_v462 import ICT_MULTI_SETUP_V462
from src.strategies.manual.reporting_v462 import load_strategy_meta
from src.strategies.manual.tmp_test_ict_multi_setup_v462 import load_nq_data


def main() -> None:
    df = load_nq_data()
    print("2) Running backtest...\n")
    bt = Backtest(df, ICT_MULTI_SETUP_V462, cash=1_000_000, commission=0.0, exclusive_orders=False, trade_on_close=False)
    stats = bt.run()
    print("3) Headline stats")
    print(stats)

    meta = load_strategy_meta(ICT_MULTI_SETUP_V462)
    if meta.empty:
    ...
    summarize(meta, ["setup_type"], "realized_dollars_dynamic_contracts", "5) By setup_type (actual in-engine 5-10 MNQ)")
    summarize(meta, ["bridge_type"], "realized_dollars_dynamic_contracts", "6) By bridge_type (actual in-engine 5-10 MNQ)")
    summarize(meta, ["setup_type", "bridge_type"], "realized_dollars_dynamic_contracts", "7) By setup_type + bridge_type (actual in-engine 5-10 MNQ)")
    summarize(meta, ["setup_tier"], "realized_dollars_dynamic_contracts", "8) By setup_tier (actual in-engine 5-10 MNQ)")
    summarize(meta, ["side"], "realized_dollars_dynamic_contracts", "9) Direction split (actual in-engine 5-10 MNQ)")
    summarize(meta, ["entry_variant"], "realized_dollars_dynamic_contracts", "10) Entry variant (actual in-engine 5-10 MNQ)")
    ...
    print("\n11) Monthly realized PnL (actual in-engine 5-10 MNQ)")
