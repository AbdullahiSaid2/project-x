...
def main() -> None:
    ...
    if "entry_time_et" in meta.columns:
        meta["entry_time_et_naive"] = pd.to_datetime(meta["entry_time_et"]).dt.tz_localize(None)
        meta["entry_month_et"] = pd.to_datetime(meta["entry_time_et"]).dt.to_period("M").astype(str)
    if "exit_time_et" in meta.columns:
        meta["exit_time_et_naive"] = pd.to_datetime(meta["exit_time_et"]).dt.tz_localize(None)
        meta["exit_month_et"] = pd.to_datetime(meta["exit_time_et"]).dt.to_period("M").astype(str)
    ...
    if "exit_month_et" in meta.columns:
        ...
        print("\n10) Monthly realized PnL at 5 MNQ")
