...
def _daily_summary(meta: pd.DataFrame, date_col: str) -> pd.DataFrame:
    out = (
        meta.groupby(date_col, dropna=False)
        .agg(
...
def main() -> None:
    ...
    if "calendar_exit_date_et" not in meta.columns:
        meta["calendar_exit_date_et"] = meta["exit_time_et"].dt.date
    ...
    cal_csv = Path("v458_daily_pnl_calendar_et.csv")
    apex_csv = Path("v458_daily_pnl_apex_session.csv")
