...
PREFERRED_BAR_SIZE = os.getenv("NQ_BAR_SIZE", "30s").strip().lower()
...
def load_nq_data() -> pd.DataFrame:
    ...
    if chosen_path is None:
        raise FileNotFoundError(f"Missing cache files: {CACHE_30S_PATH} and {CACHE_1M_PATH}")

    print(f"📦 Using local cache: {chosen_path.resolve()}")
    if chosen_path == CACHE_1M_PATH and PREFERRED_BAR_SIZE == "30s":
...
def main() -> None:
    ...
    print("\n5) Debug counters...")
    for k, v in dict(ICT_MULTI_SETUP_V461.DEBUG_COUNTERS).items():
        print(f"{k}: {v}")
