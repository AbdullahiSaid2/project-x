...
#   Win Rate  : 37.8%
#
# IDEA: Short when price gaps up at open above 20 EMA on daily chart and 5-minute moment
# ============================================================
...
class VaultStrategy(Strategy):
    """Short when gap up > 1 ATR then price reverses. Exit after 5 bars or at stop."""
    atr_mult=1.0; size=0.1; _sl=0.0; _entry_bar=-99
    ...
    def next(self):
        import numpy as np
        open_=float(self.data.Open[-1]); prev_close=float(self.data.Close[-2]); close=float(self.data.Close[-1])
        high=float(self.data.High[-1]); atr=float(self.atr[-1]); bar=len(self.data.Close)
        if np.isnan(atr): return
        if self.position.is_short:
            if high >= self._sl or bar - self._entry_bar >= 5: self.position.close()
            return
        gap_up = open_ - prev_close > atr * self.atr_mult
        if not self.position and gap_up and close < open_:
            self._sl=open_ + atr*0.5; self._entry_bar=bar; self.sell(size=self.size)
...
def run(symbol: str = "WIF", timeframe: str = "1H",
    ...
    print(f"\n📊 GapReversalShort — {symbol} {timeframe} ({days_back} days)")
