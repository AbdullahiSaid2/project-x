...
#   Win Rate  : 35.8%
#
# IDEA: Long when price crosses above 50 SMA and volume is above 20-day average, exit wh
# ============================================================
...
class VaultStrategy(Strategy):
    """Long when price breaks above 20 SMA with volume >1.5x average."""
    period=20; vol_mult=1.5; size=0.1; _sl=0.0

    def init(self):
        self.sma    = self.I(lambda c: ta_lib.trend.sma_indicator(pd.Series(c), window=self.period).values, self.data.Close)
        self.vol_ma = self.I(lambda v: pd.Series(v).rolling(self.period).mean().values, self.data.Volume)
    ...
    def next(self):
        ...
        sma=float(self.sma[-1]); vol=float(self.data.Volume[-1]); vol_ma=float(self.vol_ma[-1]); atr=float(self.atr[-1])
        if any(np.isnan(x) for x in [sma,vol_ma,atr]): return
        if self.position.is_long:
            if close < sma: self.position.close()
            return
        if not self.position and prev < sma and close > sma and vol > vol_ma * self.vol_mult:
...
def run(symbol: str = "TAO", timeframe: str = "4H",
    ...
    print(f"\n📊 SMAVolumeBreakout — {symbol} {timeframe} ({days_back} days)")
